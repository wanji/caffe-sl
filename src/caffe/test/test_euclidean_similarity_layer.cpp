...
class EuclideanSimilarityLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  EuclideanSimilarityLayerTest()
      : epsilon_(Dtype(1e-4)),
  ...
  virtual ~EuclideanSimilarityLayerTest() {
...
TYPED_TEST_CASE(EuclideanSimilarityLayerTest, TestDtypesAndDevices);

TYPED_TEST(EuclideanSimilarityLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  EuclideanSimilarityLayer<Dtype> layer(layer_param);
...
TYPED_TEST(EuclideanSimilarityLayerTest, TestGradientAcrossChannels) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  EuclideanSimilarityLayer<Dtype> layer(layer_param);
...
TYPED_TEST(EuclideanSimilarityLayerTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  EuclideanSimilarityLayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
